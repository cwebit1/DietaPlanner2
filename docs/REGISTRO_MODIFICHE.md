...
## 31. Disponibilità ricette e gestore visuale — 11 settembre 2026

**Richiesta di Cwe:** usare il file che collega ricette, fotografie e testi
anche come gestore, permettendo di impostare ogni ricetta come disponibile o
non disponibile senza cancellarla.

**Comportamento implementato:** creato `db-visuale.json` v1 con 420 record,
uno per ogni ricetta concreta identificata dall'ID stabile
`nr_<recipeModelId>_<comboIndex>`. Ogni record contiene esclusivamente
`idRicetta`, `percorsoImmagine`, `ricettaTestuale` e il booleano
`disponibile`, inizialmente `true`. Una ricetta con
`disponibile:false` resta risolvibile per dettagli, storico e pasti già
salvati, ma viene esclusa da generazione, rigenerazione, Roll e Salvafrigo.
Un ID visuale mancante resta disponibile, evitando che foto o testi incompleti
blocchino l'app.

**Gestore e sincronizzazione:** aggiunto `gestore-ricette.html`, che ricava i
nomi dal motore, consente ricerca e filtri, modifica percorso, testo e
disponibilità, segnala record incompleti e ID orfani e salva direttamente il
JSON nei browser compatibili o ne scarica una copia. Aggiunto
`tools/sincronizza-db-visuale.js` per riallineare gli ID dopo modifiche
deliberate ai gruppi preservando i dati dei record che mantengono lo stesso ID.
Il motore sincronizza in IndexedDB soltanto il flag gestionale: foto e testo
restano fuori dalla cache funzionale e dai calcoli nutrizionali.

**Verifiche riuscite:** `node --check motor-v12.js`;
`node --check tools/sincronizza-db-visuale.js`;
`node tests/catalogo-visuale-disponibilita.test.js` (420/420 ID, schema,
persistenza, esclusione, conservazione lookup e rifiuto duplicati);
`node tests/lotto-h-pwa-source-contract.test.js`;
`node tests/lotto-h-approved-recipe-conversion.test.js`;
`node tests/lotto-carboidrati-priorita-pxcuser.test.js` (100/100);
parsing degli script inline del gestore; parsing JSON; `git diff --check`.
La resa visuale del gestore non è stata dichiarata verificata in browser.
